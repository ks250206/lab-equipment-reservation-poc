...
- **アプリ DB 名・Keycloak Client ID・Python 配布名**は **`equipment-reservation` / `equipment_reservation`** に揃えています（旧 `device-reservation` から移行済み）。既存の Postgres ボリュームに **`device_reservation` データベースだけ**がある場合は、`DATABASE_URL` を新名に合わせたうえで **ボリューム削除→`just deps-up`→`just seed-dev`** が簡単です。Keycloak には **`equipment-reservation` クライアント**を新規作成するか、旧クライアントを削除してシードに任せてください。
...
5. `just seed-dev` … `ENVIRONMENT=development` のときのみ。**Keycloak が起動していることが必須**（先に Admin API でダミー利用者 8 名を作成し、その id を `users.keycloak_id` に同期してから DB に装置を投入）。続けて `equipment-reservation` クライアントと `app-admin` ロールを冪等更新（[keycloak-setup.md](keycloak-setup.md) 参照）
...
```bash
cd lab-equipment-reservation-poc
nix develop
just deps-up          # 止めていたら
...
| Postgres 対話確認（開発） | `just deps-psql`（`psql` 直起動） / `just deps-postgres-shell`（bash でコンテナに入る）。Podman 時は `podman-compose exec` が `-it` 非対応のため [scripts/postgres_interactive.sh](../scripts/postgres_interactive.sh) が `podman exec -it equipment-reservation-postgres …` に切り替える。 |
...
2. `.env` を接続先に合わせる（`DATABASE_URL`、JWT / Keycloak URL）。加えて、`compose.prod.yml` は **`POSTGRES_DB` / `POSTGRES_USER` / `POSTGRES_PASSWORD` / `KEYCLOAK_ADMIN` / `KEYCLOAK_ADMIN_PASSWORD` / `KC_DB_USERNAME` / `KC_DB_PASSWORD`** が未設定だと起動を止める。
3. `just backend-run-prod` … `ENVIRONMENT=production` で `fastapi run`
4. `just frontend-build` のあと `just frontend-preview`

Postgres ボリュームが既にあり **`keycloak` DB が無い**と Keycloak が落ちることがある → 上記「永続化プロファイル」および [docker/postgres/init/01-keycloak.sh](../docker/postgres/init/01-keycloak.sh) を参照。
...
バックエンドは `.env` の **`MINIO_ENDPOINT_URL`**（例: `http://127.0.0.1:9000`）、**`MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY`**、**`MINIO_BUCKET` / `S3_BUCKET`**（既定バケット名は `equipment-images`）等で接続する。詳細と任意の **`EQUIPMENT_IMAGE_MAX_BYTES`**（互換: `DEVICE_IMAGE_MAX_BYTES`）は [.env.example](../.env.example) を参照。MinIO が無い／接続できない場合でも API は起動するが、画像アップロードやシード画像はスキップまたは失敗ログとなる。
