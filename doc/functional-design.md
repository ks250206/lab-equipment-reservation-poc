...
| role | ENUM | DEFAULT 'user' | レガシー列（一覧表示用）。**API の管理者認可には使わない** |
| created_at | TIMESTAMP | NOT NULL | 作成日時 |
...
- **`role` 列**: 新規行は常に `user`。**管理者などの認可**は Keycloak の **レルムロール**（既定名 `app-admin`、環境変数 `KEYCLOAK_APP_ADMIN_REALM_ROLE`）が JWT の `realm_access.roles` に含まれるかで判定する。`GET /api/users/me` の JSON に返す `role` はトークン由来。一覧 `GET /api/users` の各行の `role` は DB 上の値であり、Keycloak 上の実ロールと一致しない場合がある。
- **管理者向けのユーザー一覧・更新**（`/api/users` 等）は **アプリ DB の `users` を対象**とする。`PUT` は **表示名（`name`）のみ**更新可能。Keycloak 管理コンソールの代替（パスワード変更・ユーザー無効化・ロール付与等）にはならない。
...
- 同一コマンドが Keycloak 管理 API に届く場合、**`KEYCLOAK_CLIENT_ID` の SPA クライアント**（公開クライアント・リダイレクト URI 等）を冪等に作成・更新し、レルムロール **`app-admin`** を冪等に作成して既定ユーザー（`KEYCLOAK_SEED_GRANT_APP_ADMIN_USERNAME`、既定 `admin`）へマッピングする（[doc/keycloak-setup.md](keycloak-setup.md) と整合）。Keycloak 未起動時はスキップしメッセージのみ出す。
...
| PUT | /api/users/{id} | 管理者 | ユーザー更新（アプリ DB の `name` のみ。ロールは Keycloak で管理） |
