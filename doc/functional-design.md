...
- **予約一覧（`/reservations`）**: **あなたの予約**は `GET /api/reservations` のページング・クエリ（装置・ステータス・期間・キャンセル含む・**お気に入りのみ `favorites_only`**）と **URL クエリ**を同期する。新規予約のフォームは置かず、**装置一覧／装置詳細のカレンダー**から `POST /api/reservations` で作成する。各行から **編集**（ダイアログで `PUT`）と、確定中のみ **キャンセル**（`PUT` で `cancelled`、Lucide の **Ban** アイコンのみ。文言「削除」は使わない）。**`completed` / `cancelled` 行にはキャンセル操作を出さない**。編集ダイアログは完了行は閲覧のみ（API も **409**）。**利用完了（`completed`）への遷移**は **`/reservations/usage-complete`** の専用画面から `POST /api/reservations/{id}/complete-usage` のみ（`PUT` での完了は **409**）。装置・予約の **ステータスはタグ風バッジ**で色分け表示する。
