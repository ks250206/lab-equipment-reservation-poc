...
| PUT | /api/reservations/{id} | 必須 | 予約更新（**`status` が `completed` の行は変更不可** → **409**） |
...
- **予約一覧（`/reservations`）**: **あなたの予約**は `GET /api/reservations` のページング・クエリ（装置・ステータス・期間・キャンセル含む）と **URL クエリ**を同期する。各行から **編集**（ダイアログで `PUT`）と **削除**が可能。**`completed` の行は削除ボタンを無効化**し、編集ダイアログも閲覧のみとする（API も **409** で拒否）。
