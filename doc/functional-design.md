...
- `include_cancelled`（任意、既定 `false`）: `true` のとき、`reservation_status` 未指定の場合に **キャンセル行も含める**（`reservation_status` 指定時はそのステータスで絞る）。
...
- **装置一覧（`/devices`）**: ページング UI（`ListPaginationBar`）は **検索結果リストの上**に置く。予約に基づく絞り込み（**ユーザー名・メールの一部**、**予約が重なる期間**の開始・終了の両方）を提供する。装置詳細から `?category=` / `?location=` で戻ったときは URL に合わせてフィルタを同期する。**表示モード**として **サムネ（既定）・リスト・詳細**を切り替え可能。サムネ／詳細では `GET /api/devices/{id}/image` を参照するサムネ枠を表示し、**画像なしまたは読み込み失敗時はプレースホルダ**（「画像なし」等）を出す。**フィルタ・ページ・`view`（表示モード）**はクエリ文字列と双方向同期し、ブックマーク・共有・ブラウザの戻る／進むに利用できる。
- **予約一覧（`/reservations`）**: **あなたの予約**は `GET /api/reservations` のページング・クエリ（装置・ステータス・期間・キャンセル含む）と **URL クエリ**を同期する。各行から **編集**（ダイアログで `PUT`）と **削除**が可能。
- **装置詳細の予約リスト**: 表の右端に **編集**ボタン（Lucide の鉛筆アイコン）。自分の予約は編集可能、他者は閲覧のみで同じ詳細ダイアログを開く。**カレンダー**ではイベントの**表示テキストは利用者名のみ**とし、**時刻レンジ＋名前**は要素の HTML `title`（ツールチップ）に載せる。
