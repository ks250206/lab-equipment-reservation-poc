...
| DELETE | /api/reservations/{id} | 必須 | 予約削除（**204**。**`completed` は削除不可** → **409**） |
...
- `include_cancelled`（任意、既定 `false`）: `true` のとき `cancelled` 行も含める（**`reservation_status` を指定したときは無視され、そのステータスで絞る**）。
...
- **予約一覧（`/reservations`）**: **あなたの予約**は `GET /api/reservations` のページング・クエリ（装置・ステータス・期間・キャンセル含む・**お気に入りのみ `favorites_only`**）と **URL クエリ**を同期する。新規予約のフォームは置かず、**装置一覧／装置詳細のカレンダー**から `POST /api/reservations` で作成する。各行から **編集**（ダイアログで `PUT`）と **削除**が可能。**`completed` の行は削除ボタンを無効化**し、編集ダイアログも閲覧のみとする（API も **409** で拒否）。**利用完了（`completed`）への遷移**は **`/reservations/usage-complete`** の専用画面から `POST /api/reservations/{id}/complete-usage` のみ（`PUT` での完了は **409**）。
