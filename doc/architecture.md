...
- **永続化プロファイル**: `PERSISTENCE_PROFILE=development|production` で Compose の重ね方を切替（[README.md](../README.md)）。開発では Keycloak が **dev-file**（アプリ用 Postgres とは別）、本番相当では Keycloak が **同一 Postgres 上の `keycloak` DB** に JDBC 接続する。
